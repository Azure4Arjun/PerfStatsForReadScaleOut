...
IF OBJECT_ID ('sp_perf_stats_azure','P') IS NOT NULL
   DROP PROCEDURE sp_perf_stats_azure
GO
create PROCEDURE sp_perf_stats_azure @appname sysname='PSSDIAG', @runtime datetime AS 
BEGIN 
  SET NOCOUNT ON
...
/*Added 01/30 -Rohitna*/
IF OBJECT_ID ('sp_perf_stats_azure_infrequent11','P') IS NOT NULL
   DROP PROCEDURE sp_perf_stats_azure_infrequent11
GO

CREATE PROCEDURE sp_perf_stats_azure_infrequent11
  @runtime datetime,
  @firstrun int = 0
AS
BEGIN
  ...
  SELECT @cpu_time_start = cpu_time, @elapsed_time_start = total_elapsed_time
